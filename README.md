...
- **Form editor** — instead of editing YAML by hand, walk through a step-by-step wizard (header, summary, experience, education, skills) over the known RenderCV keys. A **raw YAML toggle** on the final step keeps unknown/advanced keys reachable, and the draft autosaves to your browser's local storage between sessions.
